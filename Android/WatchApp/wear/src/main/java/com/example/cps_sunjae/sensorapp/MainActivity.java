...
public class MainActivity extends WearableActivity implements SensorEventListener,
    ...
    private static final String SENSOR_GORIENT = "sensor.gorient";
...
//    private static final float z_c = (float)-0.0942;
    ...
    private ArrayList<Long> gOrientT = new ArrayList<>();
    private ArrayList<Float> gOrientX = new ArrayList<>();
    private ArrayList<Float> gOrientY = new ArrayList<>();
    private ArrayList<Float> gOrientZ = new ArrayList<>();
    ...
    // Ignore First 20 gyro data for game orientation
    private int gyroCount = 0;
    private static final int gyroIgnore = 20;
...
//        String.format("%.3f", lAccelZ.get(lAccelT.size() - 1)));
    ...
    private void getGyroscope(SensorEvent event) {
        ...
        // Record the values
        if (start) {
            gyroT.add(System.currentTimeMillis());
            gyroX.add(gyX);
            gyroY.add(gyY);
            gyroZ.add(gyZ);

//            int orientLast = gOrientT.size()-1;
//            if (orientLast >= 0 && gyroCount >= gyroIgnore) {
//                if (Math.sqrt(gyX*gyX + gyY*gyY + gyZ*gyZ) > 0) {
//                    gOrientX.add(gOrientX.get(orientLast) + (gyX*(event.timestamp - gOrientT.get(orientLast)) * NS2S));
//                    gOrientY.add(gOrientY.get(orientLast) + (gyY*(event.timestamp - gOrientT.get(orientLast)) * NS2S));
//                    gOrientZ.add(gOrientZ.get(orientLast) + (gyZ*(event.timestamp - gOrientT.get(orientLast)) * NS2S));
//                    gOrientT.add(event.timestamp);
//                } else {
//                    gOrientX.add(gOrientX.get(orientLast));
//                    gOrientY.add(gOrientY.get(orientLast));
//                    gOrientZ.add(gOrientZ.get(orientLast));
//                    gOrientT.add(event.timestamp);
//                }
//            } else {
//                gOrientT.add(event.timestamp);
//                gOrientX.add((float)0);
//                gOrientY.add((float)0);
//                gOrientZ.add((float)0);
//                gyroCount++;
//            }
//
//            currentLabel.setText(String.format("%.3f", (gOrientX.get(gOrientX.size() - 1) * 57.2958) % 360) + "\n"
//                    + String.format("%.3f", (gOrientY.get(gOrientY.size() - 1) * 57.2958) % 360 ) + "\n"
//                    + String.format("%.3f", (gOrientZ.get(gOrientZ.size() - 1) * 57.2958) % 360));
    ...
    private void getRotationVector(SensorEvent event) {
        ...
        // Record the values
        if (start) {
            rotT.add(System.currentTimeMillis());
            rotX.add(rotVecX);
            rotY.add(rotVecY);
            rotZ.add(rotVecZ);
            rotW.add(rotVecW);
...
//      //GyroScope
        ...
        // GyroScope
//        for (int i = 0; i < gOrientT.size(); i++) {
//            gOrient = gOrient.concat(Long.toString(gOrientT.get(i)) + "\t" +
//                    Float.toString(gOrientX.get(i)) + "\t" +
//                    Float.toString(gOrientY.get(i)) + "\t" +
//                    Float.toString(gOrientZ.get(i)) + "\n");
//        }
...
//        Asset lAccelAsset = Asset.createFromBytes(lAccel.getBytes());
        ...
        Asset gOrientAsset = Asset.createFromBytes(gOrient.getBytes());

        putDataMapReq.getDataMap().putAsset(SENSOR_ACCEL, accelAsset);
        putDataMapReq.getDataMap().putAsset(SENSOR_GYRO, gyroAsset);
//        putDataMapReq.getDataMap().putAsset(SENSOR_GORIENT, gOrientAsset);
...
//        putDataMapReq.getDataMap().putAsset(SENSOR_GROTV,grvAsset);
